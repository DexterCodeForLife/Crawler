...
class Application : public Crawler::Application
{
	public :
		...
		bool onAddWebsite ( Crawler::Website & website )
		{
			...
			if ( website.getScheme ( ) != "mailto" && website != this->website )
				return false ;
		...
		{
			// Print recursively all email adresses found on the website
			for ( auto & website : this->getWebsiteManager ( ) )
			{
				if ( website.getScheme ( ) != "mailto" )
					continue ;
					
				for ( auto & link : website )
					std::cout << link.getPath ( ).substr ( 1 ) << "\n" ;
			}
